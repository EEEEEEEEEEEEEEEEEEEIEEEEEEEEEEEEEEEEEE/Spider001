...
public class ConnectManager {
	...
	public void getWebeleByType() {
		 ...
		 * ("//div[@id='TipologiaMobile']")); webElement =
		 * webElement.findElement(By.xpath(
		 * "//select[@id='ddlTipologia' and not(contains(@style,'display:none'))]"
		 * ));
		 */
		 ...
		 * 
		 * @Override public String apply(WebDriver d) { WebElement temp =
		 * d.findElement(By.xpath("//select[@id='ddlTipologia']")); Select
		 * select = new Select(temp);
		 * //select.getFirstSelectedOption().getTagName(); return
		 * select.getFirstSelectedOption().getTagName() ; }});
		 */

		// WebElement selectElement =
		// wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("formLevel:levels_input")))
		/*
	...
	private void selectOptionLevel2(WebDriver webDriver, By by, int index) {
		...
		try {
			...
			for (i = index; i < listweb.size(); i++) {
				...
				//selectType.selectByIndex(i);
				WebElement  element = listweb.get(i);
				/*JavascriptExecutor js = (JavascriptExecutor)webDriver; 
